...
def robust_telegram_webapp_validate(init_data: str):
    ...
    try:
        ...
        secret_key = hmac.new(b"WebAppData", main.BOT_TOKEN.encode(), hashlib.sha256).digest()
        calculated = hmac.new(secret_key, hmac_check.encode(), hashlib.sha256).hexdigest()
        ...
        if not valid and received_signature:
            try:
                ...
                signature_bytes = base64.urlsafe_b64decode(received_signature + "=" * (-len(received_signature) % 4))
                Ed25519PublicKey.from_public_bytes(_PROD_TELEGRAM_WEBAPP_PUBLIC_KEY).verify(signature_bytes, signature_check.encode())
                valid = True
                print("MINI APP AUTH: Ed25519 signature accepted")
            except Exception as sig_exc:
                print("MINI APP AUTH: Ed25519 signature rejected:", repr(sig_exc))
        if not valid:
            print("MINI APP AUTH FAIL: hash/signature mismatch")
            return None
        user = json.loads(user_raw)
        uid = str(user.get("id", ""))
        if not uid:
            return None
        return {"id": uid, "username": user.get("username"), "first_name": user.get("first_name"), "last_name": user.get("last_name"), "language_code": user.get("language_code")}
...
def safe_app_user_payload(uid):
    try:
        ...
        return {"id": str(uid), "name": "Student", "username": None, "submissions": 0, "obtained": 0, "max": 0, "average_percentage": 0}
    ...
    for model in main.MODELS:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                except Exception: data = json.loads(raw.replace("```json", "").replace("```", "").strip())
            ...
            if response.status_code not in (400, 404, 429, 500, 502, 503, 504): break
        except Exception as exc:
...
def threaded_run_webapp_evaluation(*args, **kwargs):
    t = threading.Thread(target=_original_run_webapp_evaluation, args=args, kwargs=kwargs, daemon=True, name="prana-evaluation-worker")
    t.start()
...
def health_check():
    return {"ok": True, "runtime": "google-cloud-run", "database_configured": bool(main.DB_ENABLED and main.SessionLocal is not None), "telegram_bot_configured": bool(main.bot), "gemini_configured": bool(main.GEMINI_API_KEY), "mini_app": True, "models": list(main.MODELS)}
